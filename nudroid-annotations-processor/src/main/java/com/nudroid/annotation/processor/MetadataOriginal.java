...
import com.nudroid.annotation.processor.model.Authority;
...
class MetadataOriginal {
     ...
     */
    MetadataOriginal(Elements elementUtils, Types typeUtils, LoggingUtils logger) {
     ...
     *             if the class is not annotated with {@link ContentProviderDelegate}.
     */
    String parseAuthorityFromClass(TypeElement delegateClass) {

        // ContentProviderDelegate authority = delegateClass.getAnnotation(ContentProviderDelegate.class);
        //
        // if (authority == null) {
        //
        // throw new IllegalArgumentException(String.format("Delegate class %s must be annotated with %s.",
        // delegateClasses.toString(), ContentProviderDelegate.class.getName()));
        // }
        //
     ...
     *            The URI to map.
     */
    void mapUri(TypeElement delegateClassType, ExecutableElement delegateMethodType, MatcherUri uri) {

        DelegateClass delegateClass = delegateClasses.get(delegateClassType.toString());

        if (delegateClass == null) {
            delegateClass = new DelegateClass(delegateClassType);
    ...
    private static class UriRegistry {
        ...
        List<MatcherUri> uris = new ArrayList<MatcherUri>();

        int addUri(MatcherUri uri) {
